...
const LanguageToggle: React.FC<{ toggleSidebar: (isOpen:boolean) => void}> = ({toggleSidebar}) => {
  ...
  return (
    <div className='relative w-fit lg:w-full'>
      ...
      />
      {dropdownOpen && (
        <DropdownMenu id="language-dropdown" options={languageOptions} closeDropdown={()=> setDropdownOpen(false)} />
      )}
    </div>
  );
};
